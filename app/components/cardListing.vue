...
<template>
  ...
  <div v-if="orientation === 'vertical'"
    class="rounded-xl justify-between w-80 h-90 flex flex-col shadow-lg relative bg-[#fafafa] dark:bg-[#1e1e1e]">
    <div class="h-1/2 w-full rounded-t-lg bg-[url('/img.png')] bg-cover bg-bottom relative">
      <button @click="heartActive = !heartActive"
        class="absolute top-3 right-3 size-8 bg-[#fafafa] dark:bg-[#3e3e3e] rounded-full flex justify-center items-center active:scale-95 hover:scale-105 transition-transform duration-200 shadow-sm">
    ...
    <div class="flex-1 h-55 flex p-3 flex-col gap-1.5">
      ...
      <div class="text-base font-medium text-[#262626] dark:text-[#e8e8e8]/80">
        $850,000<span class="text-xs font-normal text-[#262626]/80 dark:text-[#e8e8e8]/60">/month</span>
      </div>
      ...
      <div class="flex text-xs text-[#262626]/80 dark:text-[#e8e8e8]/60 gap-3 mt-1">
        <div class="flex items-center gap-1">
          <Icon name="lucide:bed" class="size-3.5" /> 4 beds
        </div>
        <div class="flex items-center gap-1">
          <Icon name="lucide:bath" class="size-3.5" /> 3 baths
        </div>
        <div class="flex items-center gap-1">
          <Icon name="lucide:ruler-dimension-line" class="size-3.5" /> 2,400 sq ft
        </div>
      </div>
      ...
      <div class="flex justify-between items-center mt-2">
        <div class="flex gap-2 items-center">
          ...
          <span class="text-xs text-[#262626]/90 dark:text-[#e8e8e8]/70">Bryl Darel G.</span>
        </div>
  ...
  <div v-else class="h-56 w-full flex shadow-lg relative bg-[#fafafa] dark:bg-[#1e1e1e] rounded-xl">
    <div class="h-full w-48 rounded-l-lg bg-[url('/img.png')] bg-cover bg-center relative">
      <button @click="heartActive = !heartActive"
        class="absolute top-3 right-3 size-8 bg-[#fafafa] dark:bg-[#3e3e3e] rounded-full flex justify-center items-center active:scale-95 hover:scale-105 transition-transform duration-200 shadow-sm">
        <Icon v-if="!heartActive" name="radix-icons:heart" class="size-5 text-gray-700 dark:text-gray-300" />
        <Icon v-else name="radix-icons:heart-filled" class="size-5 text-red-500" />

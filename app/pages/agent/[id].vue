...
<template>
  ...
  <div class="min-h-screen lg:h-screen bg-white dark:bg-[#0f1011] pt-20 lg:overflow-hidden flex flex-col">
    <div class="max-w-8xl mx-auto px-6 lg:px-12 w-full h-full flex flex-col">
      ...
      <div class="flex-1 flex flex-col lg:flex-row gap-10 lg:gap-16 lg:overflow-hidden pb-10">
        
        <aside class="w-full lg:w-1/3 shrink-0 overflow-y-auto no-scrollbar hidden-scroll px-1">
          <div class="space-y-6 pb-6 lg:pb-0">
            <div class="bg-white dark:bg-[#1a1b1e] rounded-3xl p-6 md:p-8 shadow-lg border-2 border-gray-100 dark:border-gray-800">
              <div class="flex flex-col items-center text-center">
                <div class="relative mb-6">
                  ...
                  />
                  <div class="absolute bottom-2 w-9 h-9 text-center right-2 bg-blue-600 text-white p-1.5 rounded-full shadow-lg border-2 border-white dark:border-[#1a1b1e]">
                    <Icon name="lucide:check" class="size-5 text-white" />
                  </div>
                </div>

                <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-[#fafafa] mb-1">{{ name }}</h1>
                ...
                <div class="grid grid-cols-3 w-full py-8 border-y border-gray-100 dark:border-gray-800 mb-6 text-center gap-y-8">
                  <div class="flex flex-col">
                    <span class="text-xl md:text-2xl font-black text-black dark:text-[#fafafa]">{{ reviews || '0' }}</span>
                  ...
                  <div class="flex flex-col border-x border-gray-100 dark:border-gray-800">
                    <span class="text-xl md:text-2xl font-bold text-[#262626] dark:text-[#FAFAFA] flex items-center justify-center gap-1">
                  ...
                  <div class="flex flex-col">
                    <span class="text-xl md:text-2xl font-bold text-[#262626] dark:text-[#FAFAFA]">{{ status || '0' }}</span>
                  ...
                  <div class="col-span-3 flex flex-col items-center justify-center gap-4 pt-4 border-t border-gray-50 dark:border-gray-800/50">
                    ...
                    <div class="flex items-center justify-center gap-4 flex-wrap">
                      <NuxtImg v-for="n in 3" :key="n" src="/trophy.png" class="w-10 h-10 object-contain" alt="Award Badge" />
                    </div>
                  </div>
                </div>

                <button class="w-full bg-blue-600 text-[#fafafa] py-4 rounded-xl font-semibold hover:opacity-90 transition-opacity">
                  Contact Agent
                </button>
              </div>
            </div>
        ...
        <main class="flex-1 lg:overflow-y-auto pr-0 lg:pr-4 custom-scrollbar pb-20">
          
          <section class="mb-12 md:mb-16">
            <h2 class="text-3xl md:text-4xl font-bold mb-6 md:mb-8 tracking-tight">Get to know {{ name }}</h2>
            <div class="flex flex-col gap-6 text-base md:text-lg text-gray-600 dark:text-gray-300 leading-relaxed max-w-2xl">
          ...
          <section class="mb-16 md:mb-20">
            <div class="flex items-center justify-between mb-8 md:mb-10">
              <h2 class="text-xl md:text-2xl font-bold uppercase tracking-widest">Client Reviews</h2>
              <div class="h-px flex-1 mx-4 md:mx-8 bg-gray-100 dark:bg-gray-800" />
            </div>
            
            <div v-if="displayedReviews.length" class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-10">
              <div v-for="(rev, index) in displayedReviews" :key="index" class="p-6 md:p-8 rounded-3xl border border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-zinc-900/40">

...
await propertyStore.getPropertyBySlug(slug as string)
...
const title = computed(() => property.value?.title || 'Property Detail')
...
const address = computed(() => property.value?.address || 'Address not listed')
const type = computed(() => property.value?.property_type || 'Apartments')
const user_name = computed(() => property.value?.user_name || 'Agent')
const agentPhotoSrc = computed(() => property.value?.user_avatar || '/rebecca.png')
const agentId = computed(() => property.value?.id?.toString() || 'default')

// --- EXPANDABLE IMAGE LOGIC ---
const isExpanded = ref(false)
const activeImg = ref('')
const galleryImages = computed(() => {
  const images = [propertyPhotoSrc.value]
  if (property.value?.gallery) {
    property.value.gallery.forEach((img) => {
      images.push(img.large)
    })
  }
  // Add placeholder images if gallery is empty
  if (images.length < 4) {
    images.push('/img.png', '/houseimg.png', '/img.png')
  }
    ...
    <div class="max-w-8xl mx-auto">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
          <div
            @click="expandImage(propertyPhotoSrc)"
            class="relative w-full h-125 rounded-3xl overflow-hidden shadow-2xl group cursor-zoom-in"
          >
            <div
              :style="{ backgroundImage: `url(${propertyPhotoSrc})` }"
          ...
          >
            ...
            <p class="text-gray-600 dark:text-gray-400 leading-relaxed">
              Welcome to this beautiful {{ title }} located in {{ address }}. This property features
              an expansive {{ square }} sqft floor plan with modern architecture. It offers a
              perfect balance of comfort and luxury for modern living.
            </p>
          </div>
        </div>
        ...
        <div class="lg:col-span-1">
          ...
          >
            ...
            <form class="space-y-3" @submit.prevent>
              ...
              <textarea
                rows="3"
                class="w-full p-3 rounded-xl border border-gray-200 dark:border-zinc-700 bg-transparent text-sm resize-none outline-none focus:ring-1 focus:ring-red-400"
              >
I'm interested in {{ title }}...</textarea
              >
              ...
              <div class="flex items-center gap-3 mb-4">
                ...
                <div class="flex flex-col">
                  <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider"
                    >Listed By</span
                  >
                  <span class="font-bold text-gray-900 dark:text-white leading-none">{{
                    user_name
                  }}</span>
                </div>
              </div>
              <button
                @click="navigateTo(`/agent/${agentId}`)"
                class="w-full bg-[#fe8e0a] text-[#fafafa] font-bold py-3.5 rounded-xl transition-all active:scale-95 shadow-md"
              >
